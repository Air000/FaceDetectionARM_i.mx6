 ...
 * Return Type  : void
 */
void Nondirect_stepImpl(visioncodegen_ShapeInserter *obj, unsigned char
  ...
  if (((varargin_2_size[0] == 1) || (varargin_3_size[0] == 1) ||
    ...
    for (idxROI = 0; idxROI < numShape; idxROI++) {
      ...
      lastCol = (varargin_2_data[idxROI] + varargin_2_data[idxROI + (numShape <<
                  1)]) - 2;
      if (c_obj->P0_RTP_LINEWIDTH > 1) {
        halfLineWidth = c_obj->P0_RTP_LINEWIDTH >> 1;
      ...
      if ((firstRow <= lastRow) && (firstCol <= lastCol)) {
        for (ii = 0; ii < c_obj->P0_RTP_LINEWIDTH; ii++) {
          line[0U] = firstRow + ii;
          line[1U] = firstCol;
          line[2U] = firstRow + ii;
          line[3U] = lastCol;
